...
import { CheckCircle, BookOpen, Award, Calendar, ChevronRight, Mail, AlertCircle } from 'lucide-vue-next'
import axios from 'axios'

const paymentInfo = ref(null)
const isLoading = ref(true)
const enrollmentStatus = ref('processing')
...
onMounted(async () => {
  ...
  if (enrolledCourses.value.length > 0 || (paymentInfo.value?.items?.length === 0)) {
    enrollmentStatus.value = 'success'
  } else {
    enrollmentStatus.value = 'partial'
  }
...
<template>
  ...
  <div class="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 dark:from-slate-900 dark:to-green-950 py-12">
      ...
      <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div class="p-8">
          ...
          <div v-if="isLoading" class="text-center py-12">
            <div class="animate-spin rounded-full h-12 w-12 border-b-4 border-green-600 mx-auto mb-4"></div>
            <p class="text-slate-500 dark:text-slate-400">Enrolling you in courses...</p>
          </div>
          ...
          <div v-else>
            ...
            <div class="bg-green-50 dark:bg-green-900/20 rounded-2xl p-6 mb-8">
              <div class="flex flex-col md:flex-row justify-between gap-4">
                <div>
                  ...
                  <p class="font-mono font-bold dark:text-white">{{ paymentInfo?.transactionId }}</p>
                </div>
                <div>
            ...
            <div class="space-y-3 mb-6">
              <div v-for="item in paymentInfo?.items" :key="item.id" class="flex justify-between py-3 border-b dark:border-slate-700">
                <div>
                  <p class="font-medium dark:text-white">{{ item.title }}</p>
                  <p class="text-sm text-slate-500 dark:text-slate-400">{{ item.instructor }}</p>
                </div>
            ...
            <div class="bg-gray-50 dark:bg-slate-800 rounded-2xl p-6 mb-8">
              <div class="flex justify-between pt-3 border-t border-slate-200 dark:border-slate-700">
                ...
                <span class="text-2xl font-bold text-green-600">{{ formatPrice(paymentInfo?.amount || 0) }}</span>
              </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
              <button @click="goToDashboard" class="flex-1 py-4 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition-all flex items-center justify-center gap-2">
                Go to Dashboard
                <ChevronRight class="w-4 h-4" />
              </button>
              <button @click="goToMyCourses" class="flex-1 py-4 border-2 border-green-600 text-green-600 font-bold rounded-xl hover:bg-green-50 dark:hover:bg-green-900/20 transition-all flex items-center justify-center gap-2">
